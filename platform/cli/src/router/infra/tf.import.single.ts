...
const validKeys = Object.keys(RESOURCES) as (keyof typeof RESOURCES)[];
...
const command: CLICommand = async () => {
  ...
  if (!rawKey || !validKeys.includes(rawKey as keyof typeof RESOURCES)) {
    console.error(
      `RESOURCE environment variable required. Valid values:\n${validKeys.join("\n")}`,
    );
    process.exit(1);
  }

  const key = rawKey as keyof typeof RESOURCES;
